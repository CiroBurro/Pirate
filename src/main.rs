...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!(
        "Inizializzazione completata. Trovati {} pezzi.",
        torrent.piece_picker.pieces.len()
    );
    tracing::info!(
        "Trovati {} peer dal tracker. Inizio handshake...",
        torrent.peers.len()
    );
    ...
    for mut peer in std::mem::take(&mut torrent.peers) {
        let handle = tokio::spawn(async move {
            ...
            .await
            {
                Ok(Ok(_)) => {
                    tracing::info!(
                        "Handshake COMPLETATO con successo verso: {}",
                        peer.to_string()
                    );
                }
                Ok(Err(e)) => {
                ...
                Err(_) => {
                    tracing::debug!(
                        "Timeout: il peer {} non ha risposto all'handshake",
                        peer.to_string()
                    );
                }
            }
